...
public class DBManager {
    ...
    public static ArrayList<Student> getAllActiveStudent() {
        ...
        try {
            ...
            Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/students_28_29?user=root&password=qsz9267188558d27");
    ...
    public static void modifyStudent(String id, String surname, String name, int idGroup, String date) {
        ...
        try {
            ...
            stmt.execute("UPDATE `student` SET `surname` = " +
                    "'"+surname+"', `name` = '"+name+"', `id_group` = '"
                    +idGroup+"', `date` = '"+date+"' WHERE (`id` = '"+id+"');\n");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
